...
NexT.utils = {
  ...
  registerCopyCode: function () {
    document.querySelectorAll('pre').forEach(element => {
      ...
      box.classList = 'copy-btn';
      box.innerText = '复制';
      element.append(box);
      // box.classList.add('highlight-container');
      // box.insertAdjacentHTML('afterend', '<div class="copy-btn">复制</div>');
      var button = element.querySelector('.copy-btn');
      button.addEventListener('click', event => {
        var target = event.currentTarget;
        var code = [target.parentNode.querySelector('code')].map(line => line.innerText).join();
        ...
        if (CONFIG.copycode.show_result) {
          target.innerText = result ? '复制成功' : '复制失败';
          setTimeout(() => {
            target.innerText = '复制';
          }, 1000);
        }
